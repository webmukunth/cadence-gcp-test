package com.anz.temporal.samplepayment
...
class BasicSimulation extends Simulation {
  ...
  private val scn = scenario("Basic Simulation")
    ...
    .exec(samplePayment)
    .pause(Duration(100,MILLISECONDS), Duration(200, MILLISECONDS))
  ...
  setUp(
    scn.inject(
      incrementConcurrentUsers(2) // Int
        .times(10)
        .eachLevelLasting(Duration(5, MINUTES))
        .separatedByRampsLasting(Duration(5 , SECONDS
        ))
        .startingFrom(2) // Int
    )
